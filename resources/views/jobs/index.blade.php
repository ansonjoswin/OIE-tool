                    ...
                    <div class="panel-heading"> 
                     <div  class="pull-right">
                        ...
                        <div  style="margin-right: 10px" class="pull-right">
                            <form action="{{ url('failed_jobs') }}" method="GET">{{ csrf_field() }}
                                <button type="submit" id="failed-jobs" class="btn btn-primary"><i class="fa fa-btn fa-file-o"></i>Failed Jobs</button>
                            </form>
                        </div>
   <div style="padding-left: 150px"><h4>Scheduled Jobs</h4> </div>
     ...
     <table class="table table-bordered table-striped cds-datatable">
        ...
        @foreach ($jobs as $job)
            <tr>
              ...
              <td>{{ date("Y-m-d", strtotime('+1 day', strtotime($job->created_at)+ 3600*(-6 + date("I")))). ' 01:00:00'  }}</td>   
              <td>{{date("Y-m-d h:i:s", strtotime($job->created_at) + 3600*(-6 + date("I")))}}</td>   
               <td>
       ...
       @else
    <div class="panel-body"><h4>No Scheduled Jobs found</h4></div>
     @endif
    </div>
    </div>
    </div>
